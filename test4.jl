...
function create_auth_dialog()
    ...
    set_gtk_property!(win, :focus_on_map, true)  # Ensure window gets focus when mapped
    set_gtk_property!(win, :urgency_hint, true)  # Makes the window demand attention
    ...
    function ensure_window_focus()
        present(win)  # Brings the window to the front
        grab_focus(username_entry)
    end
    
    # Handle button click
    function try_login(widget)
        ...
        if username == VALID_USERNAME && password == VALID_PASSWORD
            ...
            grab_focus(password_entry)
            ensure_window_focus()
        end
    end
    
    # Connect the login function to the button
    signal_connect(try_login, login_button, "clicked")
    
    # Handle Enter key in username field
    signal_connect(username_entry, "activate") do widget
        grab_focus(password_entry)
        return nothing
    end
    ...
    signal_connect(win, "focus-out-event") do widget, event
        ensure_window_focus()
        return false
    end
    
    # Show all components and ensure focus
    showall(win)
    ensure_window_focus()
